...
def document_chunks(document_text: str, chunk_size: int = 1000, overlap: int = 100) -> List[str]:
...
async def embeddings(
    document_chunks: List[str], embedding_model: str = "text-embedding-3-small"
) -> List[List[float]]:
    """Generate embeddings for chunks."""
    client = openai.AsyncOpenAI()
    response = await client.embeddings.create(input=document_chunks, model=embedding_model)
    return [item.embedding for item in response.data]


# 3. Vector Store
async def vector_store_ids(embeddings: List[List[float]], document_chunks: List[str]) -> List[str]:
...
async def query_embedding(
    query: str, embedding_model: str = "text-embedding-3-small"
) -> List[float]:
...
async def retrieved_chunks(query_embedding: List[float], top_k: int = 5) -> List[str]:
    """Retrieve relevant chunks from vector store."""
    import pinecone

    index = pinecone.Index("documents")

    results = await index.query(vector=query_embedding, top_k=top_k, include_metadata=True)
...
# 6. LLM Generation
async def llm_response(rag_prompt: str, model: str = "gpt-4") -> str:
    """Generate response using LLM."""
    client = openai.AsyncOpenAI()
    response = await client.chat.completions.create(
        model=model, messages=[{"role": "user", "content": rag_prompt}]
    )
...
# Indexing phase
await dr.execute(["vector_store_ids"], inputs={"document_url": "https://example.com/doc.pdf"})

# Query phase
result = await dr.execute(["llm_response"], inputs={"query": "What is the main topic?"})
```
...
@config.when(provider="openai")
async def llm_response__openai(llm_client: openai.AsyncOpenAI, prompt: str) -> str:
...
@config.when(provider="anthropic")
async def llm_response__anthropic(llm_client: anthropic.AsyncAnthropic, prompt: str) -> str:
...
async def summary(llm_client: openai.AsyncOpenAI, summarization_prompt: str) -> str:
    """Generate summary."""
    response = await llm_client.chat.completions.create(
        model="gpt-4", messages=[{"role": "user", "content": summarization_prompt}]
    )
    return response.choices[0].message.content
...
async def keywords(llm_client: openai.AsyncOpenAI, keyword_prompt: str) -> List[str]:
    """Extract keywords."""
    response = await llm_client.chat.completions.create(
        model="gpt-4", messages=[{"role": "user", "content": keyword_prompt}]
    )
...
async def sentiment(llm_client: openai.AsyncOpenAI, sentiment_prompt: str) -> str:
    """Analyze sentiment."""
    response = await llm_client.chat.completions.create(
        model="gpt-4", messages=[{"role": "user", "content": sentiment_prompt}]
    )
    return response.choices[0].message.content
...
async def llm_result(llm_client: openai.AsyncOpenAI, messages: List[dict]) -> str:
    """Execute LLM call."""
    response = await llm_client.chat.completions.create(model="gpt-4", messages=messages)
    return response.choices[0].message.content
```
...
async def structured_extraction(document: str, llm_client: openai.AsyncOpenAI) -> ExtractionResult:
...
async def tool_selection(
    llm_client: openai.AsyncOpenAI, agent_prompt: str
) -> Literal["search", "calculate", "summarize"]:
    """LLM selects which tool to use."""
    response = await llm_client.chat.completions.create(
        model="gpt-4", messages=[{"role": "user", "content": agent_prompt}]
    )
...
async def final_response(llm_client: openai.AsyncOpenAI, query: str, tool_result: str) -> str:
...
def test_structured_output():
    ...
    result = ExtractionResult(
        entities=[ExtractedEntity(name="Hamilton", type="product", relevance=0.9)],
