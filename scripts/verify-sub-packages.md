...
pip install -e .
pytest tests/ -q \
  --ignore=tests/tracking/test_ibis_stats.py \
  --ignore=tests/tracking/test_langchain_stats.py
```
...
python run.py --username voter --project-id 1
```
...
```bash
python -m venv /tmp/verify-ui && source /tmp/verify-ui/bin/activate
pip install apache-hamilton-ui==${UI_VERSION}
```
...
pip install apache-hamilton apache-hamilton-sdk==${SDK_VERSION} pandas
...
```bash
python -m venv /tmp/verify-lsp && source /tmp/verify-lsp/bin/activate
pip install "apache-hamilton[visualization]" apache-hamilton-lsp==${LSP_VERSION}
...
# The LSP reads from stdin and writes to stdout
...
assert 'capabilities' in output or 'result' in output, f'Unexpected: {output[:200]}'
print('LSP responded to initialize request')
"
```
...
```bash
python -m venv /tmp/verify-contrib && source /tmp/verify-contrib/bin/activate
pip install apache-hamilton apache-hamilton-contrib==${CONTRIB_VERSION}
...
```bash
python -m venv /tmp/verify-all && source /tmp/verify-all/bin/activate
pip install apache-hamilton==${HAMILTON_VERSION} \
...
```bash
python -m venv /tmp/verify-extras && source /tmp/verify-extras/bin/activate
pip install "apache-hamilton[sdk,lsp]"
...
```bash
python -m venv /tmp/verify-redirects && source /tmp/verify-redirects/bin/activate
pip install sf-hamilton-sdk sf-hamilton-ui sf-hamilton-lsp sf-hamilton-contrib

pip show apache-hamilton-sdk | grep Version
pip show apache-hamilton-ui | grep Version
pip show apache-hamilton-lsp | grep Version
pip show apache-hamilton-contrib | grep Version
```
